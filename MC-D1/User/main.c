...
static void nus_data_handler(ble_nus_t * p_nus, uint8_t * p_data, uint16_t length)
{
    ...
    ble_nus_string_send(&m_nus, p_data, i);

    fds_test_write(time2,i);         
    while(op_flag==true)
    {
      op_flag=false;
...
#if 0
		if (pdPASS != xTaskCreate(led_thread, "LED", 256, NULL, 3, &m_led_thread))
    {
        APP_ERROR_HANDLER(NRF_ERROR_NO_MEM);
    }
#endif
